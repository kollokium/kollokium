...
def search_video_ids(yt, keyword, target, published_after, sort_by_views):
    """변형 검색어를 돌려가며 target개를 채운다. (한 검색어로는 100개 근처에서 끊김)"""
    ids, seen = [], set()
    status = st.empty()
...
def crawl_comments(keyword, api_key, target_videos=100, max_comments=100,
    ...
    조회수 필터로 걸러질 것을 감안해 넉넉히(3배) 검색한 뒤 상위 target개를 남긴다."""
    ...
    if details:
        ...
        # 조회수 조건으로 target개를 못 채우면, 조건을 완화해 개수를 우선 확보
        ...
        st.caption(f"검색 {found}개 → 조회수 상위 {len(ids)}개 선별 (목표 {target_videos}개)")
...
with st.sidebar:
    ...
    st.caption("검색량 추이는 이 설정과 무관하게 항상 최근 250일(일별)을 수집합니다.")
    if not pred_ok:
...
if run:
    if not yt_key:
        ...
        if df.empty:
            ...
            trend_start = trend_end - pd.Timedelta(days=250)
            start = trend_start.strftime("%Y-%m-%d")
            end   = trend_end.strftime("%Y-%m-%d")
            with st.spinner("구글 검색량 추이 수집 중... (최근 250일, 일별)"):
            ...
            if not pred_ok:
                ...
                if wk.empty:
                    ...
                    st.warning("이 예측은 14개 품목으로 학습한 **프로토타입**입니다. "
                               "품목 교차검증 AUC 0.661로 무작위보다는 낫지만 실무 배포 기준에는 미치지 못합니다.")
            ...
            st.subheader("② 구글 검색량 추이 (최근 250일, 일별, 0~100)")
            if trend.empty:
