...
#   - 반응 지연     : 영상 업로드 후 댓글이 붙기까지의 시간 (열기가 식으면 길어짐)
#   - 반응 효율     : 영상 1편당 댓글 수 (영상은 나오는데 반응이 안 붙는 시점 포착)
#   - 참여 채널 수  : 몇 개 채널이 이 품목을 다루는가 (확산 폭)
#   - 신규 영상 수  : 그 주에 새로 올라온 영상 수 (공급 측 열기)
#   - 댓글 집중도   : 소수 영상에 쏠렸는지 여러 영상에 퍼졌는지 (지니계수)
#   - Bass 확산모형 : 혁신계수 p / 모방계수 q 추정, 적합도로 단일유행 여부 진단
...
# 14개 품목 학습에서 나온 생애주기 단계별 평균 — 현재 품목과 비교하는 기준선
STAGE_REF = {
...
# 기간 옵션: (댓글 수집 일수, 예측 가능 여부, 검색량 수집 일수)
# 확산 지표는 정점 전후 구간이 모두 필요하므로, 검색량은 댓글보다 넉넉히 받는다.
PERIOD_OPTIONS = {
...
def gini(x):
    """댓글이 소수 영상에 쏠린 정도 (0=고르게 분산, 1=한 영상 독점)."""
...
def bass_curve(t, p, q, M):
    """Bass 확산모형: p=혁신계수(외부 노출), q=모방계수(입소문)."""
    e = np.exp(-(p + q) * t)
    return M * ((p + q) ** 2 / p) * e / (1 + (q / p) * e) ** 2


def fit_bass(weekly_counts):
    """주간 댓글 수에 Bass 곡선을 적합해 (적합도 R2, q/p 비율)을 반환."""
...
def build_weekly_features(merged):
    """merged 데이터프레임 → 주간 확산 지표 테이블.
    학습 때와 완전히 동일한 방식으로 계산해야 예측이 맞는다.
    내용(단어)을 보지 않으므로 품목명 마스킹이 필요 없다."""
    df = merged.copy()
...
def judge_stage(row, recent_trend=0.0):
    """현재 국면 판정. 과거에 더 큰 정점이 있던 품목의 재상승을 놓치지 않도록
    최근 추세를 함께 본다."""
...
with st.sidebar:
    ...
    st.caption(f"검색량 추이는 확산 국면 판정을 위해 최근 {trend_days}일(주별)까지 수집합니다.")
    if not pred_ok:
...
if run:
    if not yt_key:
        ...
        if df.empty:
            ...
            # 검색량: 확산 국면 판정을 위해 댓글 기간보다 넉넉히 수집
            ...
            if pd.notna(cmt_start) and cmt_start < trend_start:
                trend_start = cmt_start  # 댓글이 더 오래됐으면 그에 맞춰 확장
